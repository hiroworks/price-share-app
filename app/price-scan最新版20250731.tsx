...
*/
  ...
  const [janInfo, setJanInfo] = useState<{ jan: string; genreName: string; productName: string; imageUrl: string } | null>(null);
        ...
        {step === 1 && janInfo && (
          <View>
            ...
            <View style={{ flexDirection: 'row', marginVertical: 10 }}>
              {janInfo.imageUrl ? (
                <Image source={{ uri: janInfo.imageUrl }} style={{ width: 120, height: 120, marginRight: 12 }} />
              ) : (
                <Text>画像なし</Text>
              )}
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 16, marginBottom: 8 }}>{janInfo.genreName || 'カテゴリなし'}</Text>

                <Text style={styles.inputLabel}>JANコード:</Text>
                <Text style={{ fontSize: 16 }}>{janInfo.jan || '(未取得)'}</Text>
              </View>
            </View>
            ...
            {ranking.length > 0 && (
              <>
                <Text style={[styles.heading, { marginTop: 24 }]}>🏆 価格ランキング一覧</Text>
                {ranking.slice(0, 5).map((r, i) => (
                  ...
                  >
                    ...
                    <Text style={{ fontSize: 14, color: '#666' }}>距離：{r.距離_km?.toFixed(2) ?? '?'} km</Text>
                  </TouchableOpacity>
                ))}
              </>
            )}
