    ...
    <View style={styles.container}>
      {imageUri && (
        <>
          ...
          />
          <Text>この画像でよろしいいですか？</Text>
          <Button title="OK" onPress={sendToServer} />
        </>
      )}
      ...
      <ScrollView style={{ marginTop: 20, width: '90%' }}>
        ...
        {step === 1 && janInfo && (
          <View>
            ...
            {/* 編集可能な商品名 */}
            <Text style={{ marginTop: 12 }}>商品名（編集可）:</Text>
            <TextInput
              style={styles.input}
        ...
        {step === 2 && (
          <View>
            ...
            {/* 編集可能な本体価格 */}
            <Text style={{ marginTop: 12 }}>本体価格（編集可）:</Text>
            <TextInput
              style={styles.input}
              value={finalPrice}
        ...
        {step === 3 && (
          ...
          <View>
            <Text>📍 現在のステップ: {step}</Text>
            <Text style={styles.heading}>✅ 近隣店舗を選択 (Step3)</Text>
...
console.log('選択店舗の経度（lon）：', shops[0]?.coordinates?.[0] ?? ""),
          ...
          <View>
            ...
            <Text>🛒 商品名: {finalProductName}</Text>
            <Text>📦 JANコード: {finalJanCode || '(なし)'}</Text>
            <Text>💴 値札価格: {finalPrice || '(未取得)'}</Text>
            ...
            {selectedShopIndex !== null && shops[selectedShopIndex] && (
              <Text>🏪 店舗: {shops[selectedShopIndex].name}（{shops[selectedShopIndex].distance_km?.toFixed(2)} km）</Text>
            )}
          ...
          <View>
            ...
            {ranking.map((r, i) => (
              ...
              <View key={i} style={{ marginVertical: 6 }}>
                {r.商品画像 && <Image source={{ uri: r.商品画像 }} style={{ width: 60, height: 60 }} />}
                <Text
